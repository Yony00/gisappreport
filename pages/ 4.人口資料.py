...
tainan_pop[['老幼人數比例']].plot(kind='bar', color='lightgreen', ax=ax)
plt.xlabel("行政區")
plt.ylabel("比例")
st.pyplot(fig)
...
fig, ax = plt.subplots(figsize=(6, 4))
tainan_pop[['獨居老人人數']].plot(kind='bar', color='lightblue', ax=ax)
plt.xlabel("行政區")
plt.ylabel("人數")
st.pyplot(fig)
...
fig, ax = plt.subplots(figsize=(6, 4))
tainan_pop[['低收入戶戶內人數']].plot(kind='bar', color='lightpink', ax=ax, fontproperties=font, fontsize=8)
plt.xlabel("行政區")
plt.ylabel("人數")
